...
import {
  STORAGE_KEY, DEFAULT_TARGETS, emptyState, validateEntry, calculateTotals,
...
const entry = (overrides = {}) => ({ id: 'a', name: 'Oats', calories: 500, protein: 25, fibre: 8, ...overrides });
...
test('calculates totals and remaining or over target values', () => {
  ...
  assert.deepEqual(totals, { calories: 2800, protein: 115, fibre: 38 });
...
test('legacy entries migrate and catalog metadata survives add edit delete and backup', () => {
  ...
  assert.deepEqual(parseBackup(JSON.stringify(legacy)), { ...legacy, version: 1 });
